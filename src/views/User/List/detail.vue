					...
					<el-col :span="10">
						<div class="list">
							...
							<div class="item flex">
								...
								<input class="desc flex1 col666" type="number" name="" :disabled="isEdit"
                  ...
                  <el-option
                    ...
                    :value="item.id">
								...
								<input class="desc flex1 col666" type="" name="" disabled
									v-model="basicInfo.gender==1?'女':basicInfo.gender==0?'男':'未知'" />
							</div>
							<div class="item flex">
					...
					<el-col :span="12">
						<div class="list">
							<div class="item flex">
								...
								<input class="desc flex1 col666" type="" name="" :disabled="isEditShM"
									v-model="realNameInfo.realName" />
							</div>
							<div class="item flex">
								<p class="backgroud tit">民族</p>
								<input class="desc flex1 col666" type="" name="" :disabled="isEditShM"
									v-model="realNameInfo.nation" />
							</div>
							<div class="item flex">
				...
				<el-row class="demand-deltails-box2 user">
							...
							<div class="list">
									...
									<div class="desc flex1 col666 flex alCen">
										<div v-for="item in bizCardInfo.workType"
											...
											<img v-if="isEditUserInfo" @click="detGongZhong(item)"
												src="../../../assets/images/icon-close.png" class="iconDet">
										</div>
								...
								<div class="item flex" style="position: relative;">
									...
									<input class="desc flex1 col666" type="" name="" :disabled="isEditUserInfo==false"
										v-model="bizCardInfo.selfIntroduction" />
									<el-select style="position: absolute;right: -200px;" v-model="productQuestionsVal"
										v-if="isEditUserInfo" placeholder="选择通用介绍模板" @change="chosebiref">
										<el-option v-for="item in productQuestions" :key="item.id" :label="item.content"
											:value="item.id">
										</el-option>
									</el-select>
					...
					<div class="mt10 flex alCen">
						<el-upload name="multipartFile" class="avatar-uploader" :action="adminUrl"
							...
							:on-success="workPhotoSuccess" :on-remove="handleRemove3">
					...
					<textarea v-if="isEditUserInfo" class="mt20" v-model="bizCardInfo.certificateIntro" rows="10"
						cols="80"></textarea>
			...
			getGongrenDetail() {
				...
				queryById(params).then(res => {
					...
					this.basicInfo = {
						...
						// userType:userInfo.lastUserType==0?'企业端':userInfo.lastUserType==1?'工人端':userInfo.lastUserType==2?'管理端':'',
						gender: data.gender,
						grade: data.workerGrade == 0 ? '普通工人' : data.workerGrade == 1 ? '铜牌工人' : data
							.workerGrade == 2 ? '银牌工人' : data.workerGrade == 3 ? '金牌工人' : '',
						updateTime: data.updateTime
					}
					let userType = ''
					if(data.lastUserType === 0){
						userType = '企业端'
					}else if(data.lastUserType === 1){
						userType = '工人端'
					}else if(data.lastUserType === 2){
						userType = '管理端'
					}
