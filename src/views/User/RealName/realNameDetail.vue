				...
				<el-col :span="12">
					<div class="list">
						<div class="item flex">
							...
							<p class="desc" style="color: #F59A23;">
								{{allData.status==1?'审核中':allData.status==2?'审核成功':allData.status==3?'审核失败':''}}</p>
						</div>
						<div class="item flex">
						...
						<div class="item flex">
							...
							<p class="desc">
								{{formatDate(allData.validityStartTime)}}-{{formatDate(allData.validityEndTime)}}</p>
						</div>
					</div>
				</el-col>
				<el-col :span="12">
		...
		methods: {
			loadData() {
				...
				userRealNameApplyone(data).then(res => {
					...
					this.getIdUrl(3, res.data.selfPic)
				})
			},
			seeBig(img) {
