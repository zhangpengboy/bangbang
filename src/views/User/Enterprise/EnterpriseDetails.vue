			...
			<div v-if="tabPosition == 'detail'" class="demand-deltails">
                ...
                <input  class="desc flex1 col666" disabled v-model="userIdOrType.userType==0?'企业端':userInfo.userType==1?'工人端':userInfo.userType==2?'管理端':''" />
							...
							<div class="item flex">
								...
								<el-date-picker :disabled="isEdit" v-model="userInfo.birthday" type="date"
									@change="birthChange" placeholder="选择日期">
								</el-date-picker>
							</div>
							<div class="item flex">
					...
					<el-col :span="10">
						<div class="list">
							...
							<div class="item flex">
								...
								<el-select class="desc flex1 col666" :disabled="isEdit" v-model="userInfo.gender"
									placeholder="请选择">
									<el-option v-for="item in genderoptions" :key="item.id" :label="item.labelName"
										:value="item.id">
									</el-option>
								</el-select>
							</div>
							<div class="item flex">
								...
								<input class="desc flex1 col666" type="" name=""
									v-model="renZhengInfo&&renZhengInfo.enterpriseName" disabled />
							</div>
							<div class="item flex">
								<p class="backgroud tit">注册时间</p>
								<input class="desc flex1 col666" type="" name="" disabled
					...
					<el-col :span="12">
						<div class="list">
							<div class="item flex">
								...
								<input class="desc flex1 col666" type="" name="" disabled
									:value="userInfo.realNameAuth==1?'已实名':userInfo.realNameAuth==2?'审核中':'未实名'" />
							</div>
							<div class="item flex">
								<p class="backgroud tit">性别</p>
								<!-- <input  class="desc flex1 col666" type="" name="" :disabled="isEditShM" v-model="realNameInfo&&realNameInfo.genderTxt" /> -->
								<el-select class="desc flex1 col666" :disabled="isEditShM" v-model="realNameInfo.gender"
									placeholder="请选择">
									<el-option v-for="item in genderoptions" :key="item.id" :label="item.labelName"
										:value="item.id">
									</el-option>
							...
							<div class="item flex">
								...
								<input class="desc flex1 col666" type="" name="" :disabled="isEditShM"
									v-model="realNameInfo&&realNameInfo.age" />
							</div>
							<div class="item flex">
								<p class="backgroud tit">籍贯</p>
								<input class="desc flex1 col666" type="" name="" :disabled="isEditShM"
									v-model="realNameInfo&&realNameInfo.nativePlace" />
							</div>
						</div>
					</el-col>
					<el-col :span="12">
						<div class="list">
							<div class="item flex">
								<p class="backgroud tit">姓名</p>
								<input class="desc flex1 col666" type="" name="" :disabled="isEditShM"
									v-model="realNameInfo&&realNameInfo.realName" />
							</div>
							<div class="item flex">
								<p class="backgroud tit">民族</p>
								<input class="desc flex1 col666" type="" name="" :disabled="isEditShM"
									v-model="realNameInfo&&realNameInfo.nation" />
							</div>
							<div class="item flex">
								<p class="backgroud tit">身份证号</p>
								<input class="desc flex1 col666" type="" name="" :disabled="isEditShM"
									v-model="realNameInfo&&realNameInfo.idNo" />
							</div>
							<div class="item flex">
								<p class="backgroud tit">户籍地</p>
								<input class="desc flex1 col666" type="" name="" :disabled="isEditShM"
									v-model="realNameInfo&&realNameInfo.householdRegister" />
							</div>
						</div>
					</el-col>
				</el-row>
				...
				<div class="mt15 flex">
					<div class="">
						...
						<div v-if="isEditShM">
							<img :src="realNameInfo.idCardUri" style="width: 178px;height: 178px;"
								@click="IdCardPreview(realNameInfo.idCardUri)">
						</div>
						<div v-else>
					...
					<div class="">
						<div v-if="isEditShM" style="margin-left: 20px;">
							<img :src="realNameInfo.idCardReverseUri" style="width: 178px;height: 178px;"
								@click="IdCardPreview(realNameInfo.idCardReverseUri)">
						</div>
						<div v-else>
							<el-upload class="avatar-uploader" style="margin-left: 10px;" action="123"
								...
								<img v-if="realNameInfo.idCardReverseUri" :src="realNameInfo.idCardReverseUri"
									class="avatar">
								<i v-else class="el-icon-plus avatar-uploader-icon" />
							</el-upload>
						</div>
					...
					<el-col :span="12">
						<div class="list">
							<div class="item flex">
								...
								<input class="desc flex1 col666" type="" name="" disabled
									:value="userInfo.enterpriseAuth==0?'未提交':userInfo.enterpriseAuth==1?'审核中':userInfo.enterpriseAuth==2?'已认证':userInfo.enterpriseAuth==3?'已驳回':''" />
							</div>
							<div class="item flex">
								<p class="backgroud tit">统一社会信用代码</p>
								<input class="desc flex1 col666" type="" name="" :disabled="isEditQiY"
									v-model="renZhengInfo&&renZhengInfo.businessLicenseRegistrationNo" />
							</div>
							<div class="item flex">
								<p class="backgroud tit">运营者姓名</p>
								<input class="desc flex1 col666" type="" name="" :disabled="isEditQiY"
									v-model="renZhengInfo&&renZhengInfo.operatorName" />
							</div>
							<div class="item flex">
								<p class="backgroud tit">运营者联系方式</p>
								<input class="desc flex1 col666" type="" name="" :disabled="isEditQiY"
									v-model="renZhengInfo&&renZhengInfo.operatorMobileNo" />
							</div>
						</div>
					</el-col>
					<el-col :span="12">
						<div class="list">
							<div class="item flex">
								...
								<input class="desc flex1 col666" type="" name="" :disabled="isEditQiY"
									v-model="renZhengInfo&&renZhengInfo.enterpriseName" />
							</div>
							<div class="item flex">
								<p class="backgroud tit">法人</p>
								<input class="desc flex1 col666" type="" name="" :disabled="isEditQiY"
									v-model="renZhengInfo&&renZhengInfo.legalRepresentativeName" />
							</div>
							<div class="item flex">
								<p class="backgroud tit">身份证号</p>
								<input class="desc flex1 col666" type="" name="" :disabled="isEditQiY"
									v-model="renZhengInfo&&renZhengInfo.operatorIdNo" />
							</div>
							<div class="item flex">
				...
				<div class="mt15 flex">
					<div v-if="isEditQiY">
						<img :src="item.url" v-for="(item,index) in renZhengInfo.fileUris"
							style="width: 146px;height: 146px;" @click="IdCardPreview(item.url)">
					</div>
					<div v-else>
						<el-upload name="multipartFile" :action="adminUrl" :data='{"watermarkSkip":true}'
							list-type="picture-card" :file-list="renZhengInfo.fileUris" :on-success="qiyeUpsuccess"
							:on-remove="qiyeRemove" :on-exceed="handleExceed" :limit='3'>
			...
			<div v-if="tabPosition == 'joinPro'" class="demand-service joinPro">
				...
				<div class="pro">
					<div class="flex alCen js-sb">
						<div class="flex alCen">
							<div class="flex alCen">
								<p :class="['proStatue',joinProStatus==0?'proStatueHov':'']" @click="choseProSta(0)">全部
								</p>
								<p :class="['proStatue',joinProStatus==1?'proStatueHov':'']" @click="choseProSta(1)">进行中
								</p>
								<p :class="['proStatue',joinProStatus==2?'proStatueHov':'']" @click="choseProSta(2)">已完成
								</p>
								<p :class="['proStatue',joinProStatus==3?'proStatueHov':'']" @click="choseProSta(3)">已取消
								</p>
							</div>
		...
		methods: {
			async loadDate(userInfo) {
				...
				enterQueryById(params).then(res => {
					...
					data.gradeTxt = data.workerGrade == 0 ? '普通工人' : data.workerGrade == 1 ? '铜牌' : data
						.workerGrade == 2 ? '银牌' : data.workerGrade == 3 ? '金牌工人' : ''
					data.enterpriseAuthStatusName = data.enterpriseAuthStatus == 0 ? '未提交' : data
						.enterpriseAuthStatus == 1 ? '审核中' : data.enterpriseAuthStatus == 2 ? '已通过' : data
						.enterpriseAuthStatus == 3 ? '已驳回' : ''
			...
			beforeUpload(file) {
				...
				uploadIdCardByAli(data).then(res => {
					...
					this.realNameInfo.genderTxt = this.getGenderoptions(res.data.gender)
